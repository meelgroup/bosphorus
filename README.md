...
| `gb-cone` | Gröbner bases (BRiAl's complete `symmGB_F2`, in a degree-ordered ring over the cone's variables) of cones of small equations: a cone starts from a small equation and grows by the equation sharing the most variables with it while the union stays within `--gbmaxvars` variables; short members of the reduced basis (units, equivalences, short XORs and, with `--gbfactdeg 2`, small nonlinear relations such as the implicit quadratic relations of an S-box; at most `--gbfactlen` terms) are added. A system with at most `--gbwholevars` free variables (24) is one cone and gets its complete basis even when the rule is otherwise off: random MQ systems (m = 2n) with n = 16, 20, 24, 28 are solved by the basis alone in 0.3, 1.1, 16 and 196 s, where CryptoMiniSat times out at 200 s from n = 24 on. `--gbfull 0` uses a degree-bounded (`--gbdeg`) Buchberger loop with an S-polynomial budget (`--gbsteps`) instead | `--gb 0/1`, `--gbfull`, `--gbwholevars`, `--gbmaxvars`, `--gbwindow`, `--gbmaxlen`, `--gbfactdeg`, `--gbfactlen`, `--gbdeg`, `--gbsteps` |
