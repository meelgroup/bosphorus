...
| `gb-cone` | degree-bounded Gröbner bases (BRiAl) of cones of small equations: a cone starts from a small equation and grows by the equation sharing the most variables with it while the union stays within `--gbmaxvars` variables; short members of the reduced basis (units, equivalences, short XORs and small nonlinear relations such as the implicit quadratic relations of an S-box, at most `--gbfactdeg` degree and `--gbfactlen` terms) are added. The work is bounded by a number of S-polynomials, not by time | `--gb 0/1`, `--gbdeg`, `--gbwindow`, `--gbmaxvars`, `--gbmaxlen`, `--gbsteps`, `--gbfactdeg`, `--gbfactlen` |
